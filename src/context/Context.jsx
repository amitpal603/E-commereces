...
function Context({ children }) {
  ...
  const HandleAddToCart = (singleProduct) => {
    ...
    const findIndexCurrentItem = existingProduct.findIndex(
      (cart) => cart.id === singleProduct.id
    );
    ...
    if (findIndexCurrentItem === -1) {
      ...
      existingProduct[findIndexCurrentItem] = {
        ...
        totalPrice:
          (existingProduct[findIndexCurrentItem].quantity + 1) *
          existingProduct[findIndexCurrentItem].price,
      };
    }
  ...
  const HandleRemove = (getCartItem, isFullyRemove) => {
    ...
    const findIndex = existingItem.findIndex(
    ...
    if (isFullyRemove) {
      ...
      existingItem[findIndex] = {
        ...
        totalPrice:
          (existingItem[findIndex].quantity - 1) *
          existingItem[findIndex].price,
      };
    }
